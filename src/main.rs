...
impl App {
    ...
    fn cmd_diff(&self, id1: usize, id2: usize, confidence: ConfidenceLevel) {
        ...
        let has_extras = |e: &ExtraToml| {
            e.comment.is_some()
                || !e.executors.is_empty()
                || e.suite_env.values().any(|m| !m.is_empty())
        };
        if has_extras(&extra1) || has_extras(&extra2) {
            print_extra(id1, &extra1);
            print_extra(id2, &extra2);
            println!();
        }
...
mod tests {
    ...
    fn extra_toml_loads_all_fields() {
        let extra: ExtraToml = toml::from_str(r#"
comment = "baseline"
[executors]
csom = "/opt/SOM++"
yksom = "/opt/yksom"
